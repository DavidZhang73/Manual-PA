...
Adapted from: https://github.com/AntheaLi/3DPartAssembly/blob/master/exps/exp_segmentation/models/model.py
"""
...
class Network(nn.Module):
    def __init__(self, conf, partleaf_cnt):
        ...
        self.unet = UNet(self.dim_len + 256, in_channels=3, n_classes=1, padding=True, batch_norm=True)
    ...
    def forward(self, img, pc, ins_feat, part_cnt):
        ...
        """
        pc_feat = self.pointnet(pc)  # max_parts x 512
        ...
        cond_feat = torch.cat([pc_feat, ins_feat], dim=1)  # max_parts x (512 + max_similar parts = 532)

        global_part_feat = torch.relu(self.mlp1(cond_feat))  # max_parts x 256

        global_feat = []
        t = 0

        for cnt in part_cnt:
            cur_global_feat = global_part_feat[t : t + cnt].max(dim=0, keepdim=True)[0]  # max pooling, 1 x 256
            cur_global_feat = torch.relu(self.mlp2(cur_global_feat)).repeat(cnt, 1)  # max_parts x 256
    ...
    def linear_assignment(self, mask1, mask2, similar_cnt):
        ...
        with torch.no_grad():
            while t < similar_cnt.shape[0]:
                ...
                ids1 = list(rind)
                ids2 = list(cind)
                inds1 += [int(bids[i] + ids1[i]) for i in range(len(ids1))]
                inds2 += [int(bids[i] + ids2[i]) for i in range(len(ids2))]
        return inds1, inds2
...
if __name__ == "__main__":
    ...
    input = dict(
        img=torch.rand((1, 3, 224, 224)).repeat(6, 1, 1, 1),
        pc=torch.rand((6, 1000, 3)),
        ins_feat=torch.rand((6, 20)),
        part_cnt=[6],
    )
    output = net(**input)
