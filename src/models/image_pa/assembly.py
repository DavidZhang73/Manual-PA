...
def get_surface_reweighting_batch(xyz, cube_num_point):
    ...
    out = torch.cat(
        [
            (x * y).unsqueeze(dim=1).repeat(1, np * 2),
            (y * z).unsqueeze(dim=1).repeat(1, np * 2),
            (x * z).unsqueeze(dim=1).repeat(1, np * 2),
        ],
        dim=1,
    )
...
def chamfer_distance(xyz1: torch.Tensor, xyz2: torch.Tensor, transpose: bool = False):
    return cd(xyz1, xyz2, batch_reduction=None, point_reduction=None)[0]
    ...
    def __init__(
        ...
        for i in reversed(range(depth - 1)):
            if i == depth - 2:
                ...
                self.up_path.append(UNetUpBlock(prev_channels, 0, 2 ** (wf + i), up_mode, padding, batch_norm))
            prev_channels = 2 ** (wf + i)
...
class MaskNet(nn.Module):
    def __init__(self, conf, sem_cnt):
        ...
        ## mask net work
        self.unet = UNet(self.part_dim_len + 256, in_channels=3, n_classes=1, padding=True, batch_norm=True)
    ...
    def forward(self, img, pc, ins_feat, part_cnt):
        ...
        for cnt in part_cnt:
            background = Variable(
                torch.zeros(1, self.img_size, self.img_size, dtype=torch.float), requires_grad=True
            ).to(img.device)
        ...
        for cnt in part_cnt:
            ...
            for j in range(self.num_iteration):
                node_edge_feats = torch.cat(
                    [child_feats[cur_edge_indices[:, 0], :], child_feats[cur_edge_indices[:, 1], :], cur_edge_feats],
                    dim=1,
                )
                ...
                new_child_feats = torch_scatter.scatter_mean(
                    node_edge_feats, cur_edge_indices[:, 0], dim=0, out=new_child_feats
                )
...
class PoseNet(nn.Module):
    def __init__(self, conf, sem_cnt):
        ...
        self.resnet = resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)  # pretrained
        ...
        self.mask_resnet = resnet18()  # train from scratch
        ...
        for cnt in part_cnt:
            ...
            for j in range(self.num_iteration):
                if cur_equiv_edge_indices.shape[0] > 0:
                    node_edge_feats = torch.cat(
                        [child_feats[cur_equiv_edge_indices[:, 0], :], child_feats[cur_equiv_edge_indices[:, 1], :]],
                        dim=1,
                    )
                    node_edge_feats = torch.relu(self.node_edge_op[j](node_edge_feats))
                    new_child_feats = child_feats.new_zeros(cnt, 256)
                    new_child_feats = torch_scatter.scatter_mean(
                        node_edge_feats, cur_equiv_edge_indices[:, 0], dim=0, out=new_child_feats
                    )
                    child_feats = new_child_feats
                iter_feats.append(child_feats)
            all_iter_feat = torch.cat(iter_feats, dim=1)
...
class Network(nn.Module):
    def __init__(self, conf, sem_cnt):
        ...
        self.unit_cube = torch.from_numpy(load_pts("src/models/image_pa/cube.pts")).to(conf.device)
        self.unit_anchor = torch.from_numpy(load_pts("src/models/image_pa/anchor.pts")).to(conf.device)
    ...
    def get_box_loss(self, box_size, quat1, quat2, center1, center2):
        ...
        loss_per_data = (d1 * box1_reweight).sum(dim=1) / (box1_reweight.sum(dim=1) + 1e-12) + (d2 * box2_reweight).sum(
            dim=1
        ) / (box2_reweight.sum(dim=1) + 1e-12)
        return loss_per_data
        ...
        for cur_shape_adj in adjs:
            ...
            for adj in cur_shape_adj:
                ...
                dist1, dist2 = chamfer_distance(
                    part_pcs[idx1].unsqueeze(0), part_pcs[idx2].unsqueeze(0), transpose=False
                )
...
if __name__ == "__main__":
    ...
    # ins_feat        max_parts x max_similar parts
    # similar_cnt     max_parts x 2
    # img, pc, ins_feat, part_cnt, equiv_edge_indices
    input = dict(
        img=torch.randn((1, 3, 224, 224)).repeat(6, 1, 1, 1),
        pc=torch.randn((6, 1000, 3)),
        ins_feat=torch.randn((6, 20)),
        part_cnt=torch.tensor([6]),
        equiv_edge_indices=torch.randn(6, 2).abs().int(),
    )
    output = net(**input)
    print(output)
